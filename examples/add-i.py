...
class Plant2(Plant1):
    def __init__(self):
        ...
        # User defined matrices for DFL
        self.A_cont_x   = np.array([[0.0, 0.0],
                                    [0.0, 0.0]])
...
if __name__== "__main__":
    ...
    x_0_2 = np.zeros(mplant[0].n_x)
    errs = []
    styles = ['b-.', 'r-.', 'm-.']
    labels = ['too small', 'too large', 'good size']
    for ind, plant in enumerate(mplant):
        tru2 = dm.GroundTruth(plant)
        data2 = tru2.generate_data_from_random_trajectories()
        mdmd = dm.Koopman(plant, observable='polynomial')
        mdmd.learn(data2, dmd=True)
        _, _, x_mdmd, y_mdmd = mdmd.simulate_system(x_0_2, driving_fun, tf)
        axs.plot(t, x_mdmd[:,0], styles[ind], label=labels[ind])
        errs.append(int_abs_error(x_mdmd[:,0],x_tru[:,0]))
    for ind, plant in enumerate(mplant):
        print('mDMDc{} Error: {}'.format(ind,errs[ind]))

    # plant3 = Plant3()
    # tru3 = dm.GroundTruth(plant3)
    # data3 = tru3.generate_data_from_random_trajectories()
    # mdmd = dm.Koopman(plant3, observable='polynomial')
    # mdmd.learn(data3, dmd=True)
    # _, _, x_mdmd, y_mdmd = mdmd.simulate_system(x_0_2, driving_fun, tf)
    # axs.plot(t, x_mdmd[:,0], 'c', label='mDMDc')
    # print('mDMDc Error: {}'.format(int_abs_error(x_mdmd[:,0],x_tru[:,0])))
